...
export function handKeyToParams(key: HandKey): HandParams {
  ...
  const startValue =
    pairRank === "A" ? 11 : pairRank === "T" ? 10 : parseInt(pairRank, 10)
...
export function computeStrategyTable(rules: BlackjackRules): StrategyTable {
  ...
  function computeRow(key: HandKey, params: HandParams): StrategyRow {
    ...
    for (const upcard of ALL_DEALER_UPCARDS) {
      ...
      const playComp = removeCard(
        removeCard(removeCard(baseComposition, card1), card2),
        upcard as Rank
      )
      ...
      const dealerOutcomes = getDealerOutcomesForStrategy(
        upcard,
        rules,
        playComp,
        dealerMemo
