    ...
    for (const section of [table.hard, table.soft, table.pairs]) {
      for (const row of Object.values(section)) {
        for (const cell of Object.values(row)) {
          ...
          for (const [action, ev] of Object.entries(cell.breakdown)) {
          ...
          expect(cell.action).toBe(bestAction)
        }
      }
    }
  })
  ...
  it("early surrender adds more R cells vs dealer A than vs other upcards", () => {
    ...
    for (const section of ["hard", "soft", "pairs"] as const) {
      for (const key of Object.keys(earlyTable[section])) {
        ...
        if (earlyRow["A"].action === "R" && lateRow["A"].action !== "R")
          extraVsA++
        for (const upcard of [
          "2",
          "3",
          ...
          if (earlyRow[upcard].action === "R" && lateRow[upcard].action !== "R")
            extraVsOther++
        }
      }
    }
  ...
  it("early surrender vs 2-9 upcards: identical to late surrender (no BJ possible)", () => {
    for (const upcard of ["2", "3", "4", "5", "6", "7", "8", "9"] as const) {
      for (const key of Object.keys(
        earlyTable.hard
      ) as (keyof typeof earlyTable.hard)[]) {
        expect(earlyTable.hard[key][upcard].action).toBe(
          lateTable.hard[key][upcard].action
        )
      }
    }
  })
...
describe("no surrender rules", () => {
  it("no R actions when surrender = none", () => {
    const noSurrTable = computeStrategyTable({
      ...DEFAULT_RULES,
      surrender: "none",
    })
...
describe("rule variations affect strategy", () => {
  it("disabling DAS changes some split decisions", () => {
    const withDAS = computeStrategyTable({
      ...DEFAULT_RULES,
      doubleAfterSplit: true,
    })
    const noDAS = computeStrategyTable({
      ...DEFAULT_RULES,
    ...
    for (const key of Object.keys(
      ...
      for (const upcard of [
        ...
        if (
          withDAS.pairs[key][upcard].action !== noDAS.pairs[key][upcard].action
        )
          diffs++
      }
    }
  ...
  it("double restriction 10-11 removes D actions on hard 9", () => {
    const restricted = computeStrategyTable({
      ...DEFAULT_RULES,
  ...
  it("RSA enabled: strategy table computes without error", () => {
    const rsaTable = computeStrategyTable({
      ...DEFAULT_RULES,
      resplitAces: true,
    })
